...
# ─────────────────────────────────────────────
class GenLD:
    def __init__(self, sigma: float, class_num: int, device: torch.device):
        self.inv_denom  = 1.0 / (2.0 * sigma ** 2)
        self.norm_const = 1.0 / (np.sqrt(2.0 * np.pi) * sigma)
    ...
    def __call__(self, label: torch.Tensor):
        ...
        ld   = self.norm_const * torch.exp(-dif ** 2 * self.inv_denom)
...
# ─────────────────────────────────────────────
def trainval_test(fold_idx: int, cross_val_index: str, sigma: float, lam: float):
    ...
    gen_ld = GenLD(sigma=sigma, class_num=65, device=device)
    ...
    for epoch in range(CFG['epochs']):
        ...
        logger.log({
            ...
            # 改名：elapsed_total_min 更准确（这是从训练开始的累计时间）
...
def main():
    ...
    for fold_idx, split in enumerate(cross_val_lists):
        ...
        trainval_test(fold_idx, split, sigma=30 * 0.1, lam=6 * 0.1)
