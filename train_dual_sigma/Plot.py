...
# ─────────────────────────────────────────────
# 8b. Sigma annealing schedule（sigma退火实验专用）
# baseline log里没有sigma列，函数会自动跳过
# ─────────────────────────────────────────────
def plot_sigma_schedule():
    if 'sigma' not in train.columns:
        return
    fold0 = train[train['fold'] == train['fold'].min()].sort_values('epoch')
    fig, ax = plt.subplots(figsize=(9, 4))
    ax.plot(fold0['epoch'], fold0['sigma'], color=PALETTE[2], lw=2.2)
    ax.set(xlabel='Epoch', ylabel='Sigma',
           title='Sigma Annealing Schedule — Exponential Decay (Fold 0)')
    ax.title.set_fontweight('bold')
    # 标注起点和终点值
    ax.annotate(f"σ₀ = {fold0['sigma'].iloc[0]:.2f}",
                xy=(fold0['epoch'].iloc[0],  fold0['sigma'].iloc[0]),
                xytext=(3, -0.3), textcoords='offset fontsize',
                color=PALETTE[2], fontsize=10)
    ax.annotate(f"σ_end ≈ {fold0['sigma'].iloc[-1]:.2f}",
                xy=(fold0['epoch'].iloc[-1], fold0['sigma'].iloc[-1]),
                xytext=(-8, 0.3), textcoords='offset fontsize',
                color=PALETTE[2], fontsize=10)
    save(fig, 'sigma_schedule.png')
